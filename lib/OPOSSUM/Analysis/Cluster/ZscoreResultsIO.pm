...
OPOSSUM::Analysis::Cluster::ZscoreResultsIO - Object for the I/O of
OPOSSUM::Analysis::Cluster::ZscoreResultSet objects
...
package OPOSSUM::Analysis::Cluster::ZscoreResultsIO;
...
=head2 new
 ...
 Title    : new
 Usage    : $zrIO = OPOSSUM::Analysis::Cluster::ZscoreResultsIO->new(
                -file   => $in_file
            );
 Function : Create a new OPOSSUM::Analysis::Cluster::ZscoreResultsIO object.
 Returns  : An OPOSSUM::Analysis::Cluster::ZscoreResultsIO object.
...
=head2 write_results
 ...
 Returns  : Nothing
 Args     : An OPOSSUM::Analysis::Cluster::ZscoreResultSet object
...
sub write_results
{
    ...
    if (!$results || !$results->isa("OPOSSUM::Analysis::Cluster::ZscoreResultSet")) {
        carp "no results provided or results is not a"
            . " OPOSSUM::Analysis::Cluster::ZscoreResultSet object";
        return;
    }
...
sub _write_results
{
    ...
    print $fh "TFCluster\t\tbHits\ttHits\tbRate\ttRate\tZ-score\n";

    my $num_results = $results->num_results;
    my $idx         = 0;
    while ($idx < $num_results) {
        my $result = $results->get_result($idx);
        if ($result->t_seq_hits > 0) {
            if (!$exclude_single_hits || $result->t_seq_hits > 1) {
