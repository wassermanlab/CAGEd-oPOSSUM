...
OPOSSUM::TFSet.pm - module to hold a set of TFs (TFBS::Matrix objects)

=head1 DESCRIPTION

This module replaces TFInfoSet. Instead of storing OPOSSUM::TFInfo objects,
it stores TFBS::Matrix objects. It differs from a TFBS::MatrixSet in that
...
=head2 new
 ...
 Title    : new
 Usage    : $tfs = OPOSSUM::TFSet->new();
 Function : Create a new OPOSSUM::TFSet object.
 Returns  : An OPOSSUM::TFSet object.
 Args     : None.
...
=head2 names
 ...
 Title    : names
 Usage    : $names = $tfs->names();
 Function : Return the names of the matrix objects in the set
 Returns  : A list/listref of matrix names
 Args     : None
...
sub names
{
    ...
    my @names = sort(map {$self->get_matrix($_)->name()} @$ids);
...
sub get_matrix_list
{
    ...
    if ($sort_field) {
        ...
        } elsif ($sort_field eq 'NAME' || $sort_field =~ /_NAME$/) {
            # case insensitive sort
            @matrix_list = sort {uc $a->name() cmp uc $b->name()} @matrix_list;
        }
    }
