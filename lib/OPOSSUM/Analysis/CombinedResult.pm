...
=head2 new
 ...
 Usage    : $result = OPOSSUM::Analysis::CombinedResult->new(
                -id                => $id,
                -t_seq_hits        => $t_seq_hits,
                -bg_seq_hits       => $bg_seq_hits,
                -t_seq_no_hits     => $t_seq_no_hits,
                -bg_seq_no_hits    => $bg_seq_no_hits,
 ...
 Args     : id              - Result ID, i.e. the TF ID
            t_seq_hits      - Number of sequences in the target set which had
                              at least one binding site for this TF
            t_seq_no_hits   - Number of sequences in the target set which did
                              not have at least one binding site for this
                              TF
            bg_seq_hits     - Number of sequences in the background set which
                              had at least one binding site for this TF
            bg_seq_no_hits  - Number of sequences in the background set of
                              which did not have at least one binding site
                              for this TF
            t_tfbs_hits     - Total number of binding sites for this TF in
                              the target set of sequences
            bg_tfbs_hits    - Total number of binding sites for this TF in
                              the background set of sequences
            ...
            zscore          - Z-score measure of significance of number
                              ...
                              background set of sequences for this TF.
            ...
            fisher_p_value  - Fisher probability measure of number of
                              target sequences which had at least one site for
...
=head2 t_seq_hits
 ...
 Title    : t_seq_hits
 Usage    : $seq_hits = $result->t_seq_hits();
            $result->t_seq_hits($seq_hits);
 Function : Get/set the number of target sequences for which at least one
            binding site of this TF was detected
 Returns  : Target sequence hits
 Args     : Optional target sequence hits
...
sub t_seq_hits
{
    ...
    if (defined $hits) {
        $self->{-t_seq_hits} = $hits;
    }

    return $self->{-t_seq_hits};
}

=head2 t_seq_no_hits

 Title    : t_seq_no_hits
 Usage    : $seq_no_hits = $result->t_seq_no_hits();
            $result->t_seq_no_hits($seq_no_hits);
 Function : Get/set the number of target sequences for which no binding
            sites of this TF were detected
 Returns  : Target sequence non-hits.
 Args     : Optional target sequence non-hits.
...
sub t_seq_no_hits
{
    ...
    if (defined $hits) {
        $self->{-t_seq_no_hits} = $hits;
    }

    return $self->{-t_seq_no_hits};
}

=head2 bg_seq_hits

 Title    : bg_seq_hits
 Usage    : $seq_hits = $result->bg_seq_hits();
            $result->bg_seq_hits($seq_hits);
 Function : Get/set the number of target sequences for which at least one
            binding site of this TF was detected
 Returns  : Background sequence hits.
 Args     : Optional background sequence hits.
...
sub bg_seq_hits
{
    ...
    if (defined $hits) {
        $self->{-bg_seq_hits} = $hits;
    }

    return $self->{-bg_seq_hits};
}

=head2 bg_seq_no_hits

 Title    : bg_seq_no_hits
 Usage    : $seq_no_hits = $result->bg_seq_no_hits();
            $result->bg_seq_no_hits($seq_no_hits);
 Function : Get/set the number of background sequences for which no binding
            sites for this TF were detected
 Returns  : Background sequence non-hits.
 Args     : Optional background sequence non-hits.
...
sub bg_seq_no_hits
{
    ...
    if (defined $hits) {
        $self->{-bg_seq_no_hits} = $hits;
    }

    return $self->{-bg_seq_no_hits};
}
...
=head2 t_tfbs_hits
 ...
 Function : Get/set the number of binding sites for this TF which were
            detected in the target set of sequences
 Returns  : Target sequence TF binding sites
 Args     : Optional target sequence TF binding sites
...
=head2 bg_tfbs_hits
 ...
 Function : Get/set the number of binding sites for this TF which were
            detected in the background set of sequences
 Returns  : Background sequence TF binding sites
 Args     : Optional background sequence TF binding sites
...
=head2 t_tfbs_rate
 ...
 Function : Get/set the rate of binding site nucleotides for this TF
            which were detected in the target set of sequences
 Returns  : Target sequence TF binding site nucleotide rate
 Args     : Optional target sequence TF binding site nucleotide rate
...
=head2 bg_tfbs_rate
 ...
 Function : Get/set the rate of binding site nucleotides for this TF
            which were detected in the background set of sequences
 Returns  : Background sequence TF binding site nucleotide rate
 Args     : Optional background sequence TF binding site nucleotide rate
