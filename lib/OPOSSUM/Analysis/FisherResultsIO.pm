...
OPOSSUM::Analysis::FisherResultsIO - Object for the I/O of
OPOSSUM::Analysis::FisherResultSet objects
...
package OPOSSUM::Analysis::FisherResultsIO;
...
=head2 new
 ...
 Title    : new
 Usage    : $frIO = OPOSSUM::Analysis::FisherResultsIO->new(
                -file	=> $in_file
            );
 Function : Create a new OPOSSUM::Analysis::FisherResultsIO object.
 Returns  : An OPOSSUM::Analysis::FisherResultsIO object.
...
=head2 write_results
 ...
 Function : Write results to the open filehandle. If $exclude_single_hits
            ...
            found for a single sequence.
 Returns  : Nothing
 Args     : An OPOSSUM::Analysis::FisherResultSet object
            Optionally a flag which, if true, excludes results in which
            a TFBS was only found for a single sequence
...
sub write_results
{
    ...
    if (!$results || !$results->isa("OPOSSUM::Analysis::FisherResultSet")) {
        carp "no results provided or results is not a"
            . " OPOSSUM::Analysis::FisherResults object";
        return;
    }
...
sub _read_results
{
    ...
    my $results = OPOSSUM::Analysis::FisherResultSet->new;
    if (!$results) {
        ...
        && $headers[5] eq 'p-value')
    {
        while (my $line = <$fh>) {
            ...
            {
                $results->add_result(
                    OPOSSUM::Analysis::FisherResult->new(
                        -id         => $vals[0],
                        -t_hits     => $vals[1],
                        -t_no_hits  => $vals[2],
                        -bg_hits    => $vals[3],
...
sub _write_results
{
    ...
    print $fh "TF\t\tbHits\tbNoHits\ttHits\ttNoHits\tp-value\n";
