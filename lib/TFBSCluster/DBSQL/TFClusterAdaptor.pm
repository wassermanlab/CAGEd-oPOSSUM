...
#
# Added new generic fetch which parses the fetch criteria arguments and calls
# appropriate routine.
# DJA 2016/4/13
#
sub fetch
{
    ...
    if ($args) {
        ...
        # These ones aren't actually used. DJA 2016/4/13
        my $class = $args->{-classes};
        my $family = $args->{-families};

        if ($tf_ids) {
            # Specific TF IDs take precendce over all other criteria
            $tf_clusters = $self->fetch_by_tf_id_list($tf_ids);
        } elsif ($min_ic) {
            $tf_clusters = $self->fetch_by_tf_min_ic($min_ic);
        } elsif ($class) {
            $tf_clusters = $self->fetch_by_tf_classes($class);
        } elsif ($family) {
            $tf_clusters = $self->fetch_by_tf_families($family);
        } else {
            $tf_clusters = $self->fetch_all();
        }
    } else {
        $tf_clusters = $self->fetch_all();
    }
...
sub fetch_where
{
    ...
    while (my @row = $sth->fetchrow_array) {
        my $tf_cluster = TFBSCluster::TFCluster->new(
            ...
            # Not used anymore DJA 2016/04/12
            -class	=> $row[2],
            -family	=> $row[3]
        );
...
=head2 fetch_by_tf_min_ic

 Title    : fetch_by_tf_min_ic
 Usage    : $clset = $tfca->fetch_by_tf_min_ic($min_ic);
...
sub fetch_by_tf_min_ic
{
    my ($self, $min_ic) = @_;

    my $sql = qq{select distinct(cluster_id) from tfs where IC >= ?};
    ...
    my @clusters;
    if (!$sth->execute($min_ic)) {
        carp "error fetching cluster_ids with min IC = $min_ic\n"
            . $self->errstr;
        return;
    }
