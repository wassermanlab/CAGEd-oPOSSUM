...
OPOSSUM::ZscoreResult.pm - module to hold the result of a Zscore analysis for
a single TFBS
...
package OPOSSUM::Analysis::ZscoreResult;
...
=head2 new
 ...
 Title    : new
 Usage    : $zr = OPOSSUM::Analysis::ZscoreResult->new(
                -id             => $id,
                -t_hits         => $t_hits,
                -bg_hits        => $bg_hits,
                -t_rate         => $t_rate,
                ...
                -z_score        => $z_score,
                -p_value        => $p_value);
 Function : Create a new OPOSSUM::Analysis::ZscoreResult object.
 Returns  : An OPOSSUM::Analysis::ZscoreResult object.
 Args     : id      - TFBS ID
            t_hits  - number of times this TFBS was detected in the
                      target set of sequences
            bg_hits	- number of times this TFBS was detected in the
                      background set of sequences
            t_rate	- rate that this TFBS's nucleotides were detected
                      in the target set of sequences
            bg_rate	- rate that this TFBS's nucleotides were detected
                      in the background set of sequences
            t_seq_hits	- number of sequences in the target set for which this
                      TFBS was detected
            bg_seq_hits - number of sequences in the background set for which
...
sub new
{
    ...
    my $t_seq_hits   = $args{-t_seq_hits};
    my $bg_seq_hits  = $args{-bg_seq_hits};
...
=head2 t_seq_hits
 ...
 Title    : t_seq_hits
 Usage    : $t_seq_hits = $zr->t_seq_hits()
            or $zr->t_seq_hits($t_seq_hits);
 Function : Get/set the target sequence hits of this result.
 Returns  : integer target sequence hits.
 Args     : Optional integer target sequence hits.

=cut

sub t_seq_hits
{
    my ($self, $t_seq_hits) = @_;

    if (defined $t_seq_hits) {
        $self->{-t_seq_hits} = $t_seq_hits;
    }
...
=head2 bg_seq_hits
 ...
 Title    : bg_seq_hits
 Usage    : $bg_seq_hits = $zr->bg_seq_hits()
            or $zr->bg_seq_hits($bg_seq_hits);
 Function : Get/set the background sequence hits of this result.
 Returns  : integer background sequence hits.
 Args     : Optional integer background sequence hits.

=cut

sub bg_seq_hits
{
    my ($self, $bg_seq_hits) = @_;

    if (defined $bg_seq_hits) {
        $self->{-bg_seq_hits} = $bg_seq_hits;
    }
