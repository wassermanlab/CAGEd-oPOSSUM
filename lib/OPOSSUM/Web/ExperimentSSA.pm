...
my $log_file = "$log_dir/F5OP_exp_ssa";
...
sub input
{
    ...
    #
    my @in_t_exp_ids = @{$state->t_exp_ids()} if $state->t_exp_ids();
...
sub process
{
    ...
    #
    my $command = OPOSSUM_SCRIPTS_PATH . "/experiment_ssa.pl"
        . " -j " . $self->param('job_id')
        . " -m " . $self->param('email')
    ...
    if ($self->param('t_input_method') eq 'experiment') {
        my @exp_ids = map {$_->id} @{$self->param('t_experiments')};

        $command .= " -txids " . join ',', @exp_ids;

        if (defined $self->param('t_tag_count')) {
            $command .= " -ttc " . $self->param('t_tag_count');
        }

        if (defined $self->param('t_tpm')) {
            $command .= " -ttpm " . $self->param('t_tpm');
        }
    } else {
        $command .= " -ttssf " . $self->param('t_tss_names_file');

        if (defined $self->param('t_user_tss_names_file')) {
            $command .= " -uttssf " . $self->param('t_user_tss_names_file');
        }
    }

    if (defined $self->param('t_use_tss_only')) {
        $command .= " -tto";
    }

    if (defined $self->param('t_gene_ids_file')) {
        $command .= " -tgf " . $self->param('t_gene_ids_file');
        $command .= " -utgf " . $self->param('t_user_gene_ids_file')
            if $self->param('t_user_gene_ids_file');
    }
    ...
    if ($self->param('b_input_method') eq 'experiment') {
    ...
    my $vars = {
        ...
        t_experiments       => $self->param('t_experiments'),
        b_experiments       => $self->param('b_experiments'),
        t_tag_count         => $self->param('t_tag_count'),
        t_tpm               => $self->param('t_tpm'),
        b_tag_count         => $self->param('b_tag_count'),
        b_tpm               => $self->param('b_tpm'),
        t_user_gene_ids_file => $self->param('t_user_gene_ids_file'),
        b_user_gene_ids_file => $self->param('b_user_gene_ids_file'),
        ...
        t_user_tss_names_file => $self->param('t_user_tss_names_file'),
        ...
        var_template        => "analysis_summary_experiment_ssa.html"
    };
...
sub process_cgi_query
{
    ...
    my $t_input_method;
    if (my $text = $self->parse_textbox('t_tss_names_text')) {
        ...
        $self->param('t_tss_names_file', $filename);

        $t_input_method = 'tss_names';
    } elsif (
        my $upload_filename = $self->parse_upload_filename('t_tss_names_file')
    ) {
        my $filename = "$results_dir/t_tss_names.txt";

        unless (
            $self->create_local_file_from_upload($filename, 't_tss_names_file')
        ) {
            $self->_error(
                "Could not create local target TSS names file $filename"
            );
        ...
        $self->param('t_tss_names_file', $filename);
        $self->param('t_user_tss_names_file', $upload_filename);

        $t_input_method = 'tss_names';
    } else {
        # Just a placeholder for now, check/process later...
        $t_input_method = 'experiment';
    }

    my $b_input_method;
    if (my $text = $self->parse_textbox('b_tss_names_text')) {
    ...
    my $all_experiments;
    if ($t_input_method eq 'experiment' || $b_input_method eq 'experiment') {
        my $expa = $opdba->get_ExperimentAdaptor();

        $all_experiments = $expa->fetch_where();
    ...
    if ($t_input_method eq "experiment") {
        my @t_experiments;
        foreach my $exp (@$all_experiments) {
            if ($q->param('t_' . $exp->FF_id)) {
                push @t_experiments, $exp;
            }
        }

        unless (@t_experiments) {
            $self->_error("No target experiments or TSSs specified");
            return;
        }

        $self->param('t_experiments', \@t_experiments);

        my $t_tag_count = $self->parse_textbox('t_tag_count');
        unless (defined $t_tag_count) {
            $t_tag_count = DFLT_TARGET_TAG_COUNT;
        }

        $self->param('t_tag_count', $t_tag_count);

        my $t_tpm = $self->parse_textbox('t_tpm');
        unless (defined $t_tpm) {
            $t_tpm = DFLT_TARGET_TPM;
        }

        $self->param('t_tpm', $t_tpm);

        $self->param('t_use_tss_only', $q->param('t_use_tss_only'));

        if (my $text = $self->parse_textbox('t_gene_ids_text')) {
            my $filename = "$results_dir/t_gene_ids.txt";

            unless ($self->create_local_file_from_text($filename, $text)) {
                $self->_error(
                    "Could not create local target gene IDs file $filename"
                );
                return;
            }

            $self->param('t_gene_ids_file', $filename);
        } elsif (
            my $upload_filename
                = $self->parse_upload_filename('t_gene_ids_file')
        ) {
            my $filename = "$results_dir/t_gene_ids.txt";

            unless($self->create_local_file_from_upload(
                $filename, 't_gene_ids_file'
            )) {
                $self->_error(
                    "Could not create local target gene IDs file $filename"
                );
            ...
            $self->param('t_gene_ids_file', $filename);
            $self->param('t_user_gene_ids_file', $upload_filename);
        }
    ...
    $self->param('t_input_method', $t_input_method);

    if ($b_input_method eq "experiment") {
        my @b_experiments;
        foreach my $exp (@$all_experiments) {
...
sub initialize_state
{
    ...
    my $heading = sprintf "%s Experiment Single Site Analysis",
