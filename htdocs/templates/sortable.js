...
var image_path = "http://opossum.cisreg.ca/oPOSSUM3/images/";
...
function ts_makeSortable(table) {
	...
	for (var i=0;i<firstRow.cells.length;i++) {
		...
		if (cell.className != "unsortable" && cell.className.indexOf("unsortable") == -1) {
			cell.innerHTML = txt + '<a href="#" class="sortheader" onclick="ts_resortTable(this, '+i+');return false;">'+'<span class="sortarrow">&nbsp;<img src="'+ image_path + image_none + '" alt="&darr;" border="0"/></span></a>';
		}
	}
	...
	for (var ci=0;ci<allspans.length;ci++) {
		if (allspans[ci].className == 'sortarrow') {
			if (getParent(allspans[ci],"table") == getParent(lnk,"table")) { // in the same table as us?
				allspans[ci].innerHTML = '&nbsp<img src="'+ image_path + image_none + '" alt="&darr;"/>';
			}
		}
	}
