...
OPOSSUM::CombinedResultSet.pm - module to hold the results of a Combined analysis
...
package OPOSSUM::Analysis::CombinedResultSet;
...
use OPOSSUM::Analysis::CombinedResult;

=head2 new

 Title    : new
 Usage    : $crs = OPOSSUM::Analysis::CombinedResultSet->new();
 Function : Create a new OPOSSUM::Analysis::CombinedResultSet object.
 Returns  : An OPOSSUM::Analysis::CombinedResultSet object.
 Args     : None.
...
=head2 add_result
 ...
 Returns  : The OPOSSUM::Analysis::CombinedResult object just added
 Args     : An OPOSSUM::Analysis::CombinedResult object
...
sub add_result
{
    ...
    unless ($result->isa("OPOSSUM::Analysis::CombinedResult")) {
        carp "Argument is not an OPOSSUM::Analysis::CombinedResult\n";
        return;
    }
...
=head2 get_list
 ...
 Returns  : A listref of OPOSSUM::Analysis::CombinedResult objects
...
sub _combine_results
{
    ...
    foreach my $id (@fids, @zids) {
        ...
        if ($fresult) {
            $cresult->t_seq_hits($fresult->t_hits());
            $cresult->bg_seq_hits($fresult->bg_hits());
            $cresult->t_seq_no_hits($fresult->t_no_hits());
            $cresult->bg_seq_no_hits($fresult->bg_no_hits());
            $cresult->fisher_p_value($fresult->p_value());
        }

        if ($zresult) {
            $cresult->t_tfbs_hits($zresult->t_hits());
            $cresult->bg_tfbs_hits($zresult->bg_hits());
            $cresult->t_tfbs_rate($zresult->t_rate());
            $cresult->bg_tfbs_rate($zresult->bg_rate());
