...
=head1 SYNOPSIS
  ...
  fetch_gene_info.pl -i gene_ids_file -s species -d ensembl_db_name
...
=head1 ARGUMENTS
  ...
  -i gene_ids_file       = Name of input Ensembl gene stable IDs file
  -s species             = Name of species (common name, e.g. 'mouse')
  ...
  -op out_promoters_file = Output promoters file
...
Given a list of input Ensembl gene stable IDs, connect to the specified
Ensembl database and extract the corresponding gene/exon/promoter/seq
information needed to populate the oPOSSUM genes/exons/promoters/sequences
tables. Output the information to the corresponding tab delimited files
suitable for loading into oPOSSUM using mysqlimport.
...
# Ensembl API lib
#
# For last human assembly 37
#use lib '/usr/local/src/ensembl-75/ensembl/modules';
# For last mouse assembly 37
use lib '/usr/local/src/ensembl-67/ensembl/modules';
...
#<IN>; # skip header
my $gene_id = 1;
while (my $stable_id = <IN>) {
    chomp $stable_id;

    $logger->info(sprintf("Processing gene %7d\t%s", $gene_id, $stable_id));

    my $ens_gene = $ga->fetch_by_stable_id($stable_id, 1);

    if (!$ens_gene) {
        $logger->error("fetching Ensembl $species gene $stable_id");
        next;
    }
    ...
    print OGFH "$gene_id\t$stable_id\t$gene_name\t$gene_biotype\t$gene_chr"
