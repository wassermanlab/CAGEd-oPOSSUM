...
Given TFBS position weight matrix counts for each sequence in a target and
background set, calculate the probability that each binding site is
overrepresented in the set of target sequences as a z-score.
...
where,
 ...
 Ne = number of expected binding site nucleotides in the target set of
      sequences and is equal to the number of observed binding site
      nucleotides in the backgtound set of sequences multiplied by the ratio
...
=head2 calculate_Zscore
 ...
 Args     : bg_counts   - An OPOSSUM::Analysis::Counts object containing
                          the TFBS counts in the background set of sequences
            t_counts    - An OPOSSUM::Analysis::Counts object containing
                          the TFBS counts in the target set of sequences
...
sub calculate_Zscore
{
    ...
    foreach my $tf_id (@$t_tf_ids) {
        ...
        #if (defined $Z) {
            my $t_seq_hits  = $t_counts->tfbs_seq_count($tf_id);
            my $bg_seq_hits = $bg_counts->tfbs_seq_count($tf_id);
            $results->add_result(
